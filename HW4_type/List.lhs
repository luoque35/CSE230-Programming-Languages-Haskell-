...
replicate 0   _ = Emp
replicate cnt x = x :+: replicate (cnt - 1) x
\end{code}
...
{-@ link :: xs:List a -> ys:List a-> {z:List a | size z == (size xs + size ys) } @-}
link :: List a -> List a -> List a
link (x :+: xs) ys = x :+: link xs ys
link Emp        ys = ys



{-@ concat :: xs:List (List a) -> ListN a (sizeList xs)@-}
concat Emp = Emp
concat (x :+: xs) = link x (concat xs)
