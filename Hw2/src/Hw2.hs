...
myReverse w@(x:xs) = foldl (\xs x -> [x] ++ xs) [] w
myReverse (x) = x
...
delete _ Emp                = Emp
delete k (Bind t v Emp Emp)
  | k == t                  = Emp
  | otherwise               = Bind t v Emp Emp
delete k (Bind t v l r)
  | k < t                   = Bind t v (delete k l) r
  | k > t                   = Bind t v l (delete k r)
  | k == t                  = case (l,r) of
                                  (Emp, (Bind _ _ _ _)) -> r
                                  ((Bind _ _ _ _), Emp) -> l
                                  otherwise -> Bind (fst right) (snd right) (delete (fst right) l) r
