...
getValFromMap :: (MonadState Store m, MonadError Value m) => Store -> Variable -> m Value
getValFromMap map var =
    let result = (Map.lookup var map) in
        case result of
            Just value -> return value
            otherwise -> throwError (IntVal 0)
...
evalOp :: (MonadState Store m, MonadError Value m) => Bop -> Value -> Value -> m Value
...
evalOp Divide (IntVal i) (IntVal j)
    ...
    | otherwise = return $ IntVal (i `div` j)
...
--   statement `h` is executed.
evalS (Try s x h)    = do{
                        evalS s `catchError` ( \err -> do{
                          evalS (Assign x $ Val err);
                          evalS h;
                          return ();
                        })
                      }
...
-- then `execute (Map.fromList []) testprog1` should return the triple
...
-- then `execute (Map.fromList []) testprog2` should return the triple
...
insUnbal :: (Ord k) => k -> v -> BST k v -> BST k v
insUnbal k v Emp               = Bind k v Emp Emp
insUnbal k v (Bind k0 v0 l r)
  | k < k0                  = Bind k0 v0 (insUnbal k v l) r
  | k > k0                  = Bind k0 v0 l (insUnbal k v r)
  | k == k0                 = Bind k v l r

bstInsert :: (Ord k) => k -> v -> BST k v -> BST k v
bstInsert k v t = bstBalanced . toBinds $ insUnbal k v t


split :: (Ord k) => [(k,v)] -> ( k, v, [(k,v)], [(k,v)])
split l = let len = length l
              ll  = take (len `div` 2) $ l
              rr  = drop 1 . drop (len `div` 2) $ l
              (rk, rv) = l !! (len `div` 2)
          in  (rk, rv, ll, rr)



bstBalanced :: (Ord k) => [(k,v)] -> BST k v
bstBalanced l
  | (length l) == 0  = Emp
  | (length l) == 1  = let (k,v) = l !! 0
                       in (Bind k v Emp Emp)
  | (length l) > 1   = let (rk, rv, ll, rr) = split l
                       in (Bind rk rv (bstBalanced ll) (bstBalanced rr))
...
rightMost :: BST k v -> (k , v)
rightMost Emp = error "Empty Node"
rightMost (Bind t v _ Emp) = (t, v)
rightMost (Bind _ _ _ r) = rightMost r

delUnbal :: (Ord k) => k -> BST k v -> BST k v
delUnbal _ Emp                = Emp
delUnbal k (Bind t v Emp Emp)
  | k == t                  = Emp
  | otherwise               = Bind t v Emp Emp
delUnbal k (Bind t v l r)
  | k < t                   = Bind t v (delUnbal k l) r
  | k > t                   = Bind t v l (delUnbal k r)
  | k == t                  = case (l,r) of
                                  (Emp, (Bind _ _ _ _)) -> r
                                  ((Bind _ _ _ _), Emp) -> l
                                  otherwise -> Bind (fst right) (snd right) (delUnbal (fst right) l) r
                                              where right = (rightMost l)


bstDelete :: (Ord k) => k -> BST k v -> BST k v
bstDelete k t = bstBalanced . toBinds $ delUnbal k t
...
-- genBal = Gen <$> ( bstBalanced [(k,v) | k <- [1..5], v <- ['a'..'g']])
genBal = do {
            maxN <- elements [1..10];
            l <- sublistOf [(k,v) | k <- [1..maxN], v <- ['a'..'g']];
            return (bstBalanced l);
        }
...
impp2 :: (Signal, Signal) -> Signal
impp2 = lift2 $ \x y -> (not x) && y
...
prop_bitSubtractor_Correct  cin xs
  | (binary xs) >= (binary $ sample1 cin) = let (out, bout) = bitSubtractor (cin, map lift0 xs)
                                                ll = binary (sampleN out)
                                                rr = (binary xs) - (binary $ sample1 cin)
                                             in ll == rr
  | otherwise                             = let (out, bout) = bitSubtractor (cin, map lift0 xs)
                                                ll = binary (sampleN out)
                                                rr = 0
                                             in ll == rr
...
fullSub (cin, [])     = ([], cin)
fullSub (cin, x:xs)   = let (diff, borrow)= halfsub (x, cin)
                            (diffs, cout) = fullSub (borrow, xs)
                        in  (diff:diffs ,cout)

bitSubtractor :: (Signal, [Signal]) -> ([Signal], Signal)
bitSubtractor (cin, [])     = ([], cin)
bitSubtractor (cin, x:xs)   = let  (diff, borrow)      = halfsub (x, cin)
                                   (diffs, cout)       = fullSub (borrow, xs)
                              in (muxN (repeat low, diff:diffs, cout) , low)
...
prop_Multiplier_Correct l1 l2 =   binary (sampleN sum) == binary l1 * binary l2
                            where sum = multiplier (map lift0 l1, map lift0 l2)
...
bitMultiplier :: ([Signal], Signal, [Signal], [Signal]) -> ([Signal], [Signal])
bitMultiplier ([],_,_,_) = ([],[])
bitMultiplier (x:xs, y, s:sins, c:cins) = let (sout,cout)   = fulladd (c, s, (and2 (x, y)))
                                              (souts,couts) = bitMultiplier (xs, y, sins, cins)
                                          in  (sout:souts, cout:couts)


multi :: ([Signal], [Signal], [Signal], [Signal]) -> [Signal]
multi (xs, [], sin, cin) = adder (sin, cin)
multi (xs, y:ys, sin, cin) = let (souts, couts) = bitMultiplier (xs, y, sin, cin)
                                 hi = [head souts]
                                 lo = multi (xs, ys, (tail souts) ++ [low], couts)
                             in hi ++ lo
...
multiplier (_,[])   = []
multiplier ([],_)   = []
multiplier (xs, bs) = multi (xs, bs, repeat low, repeat low)
